...
public class Skeleton extends WurstplusHack {
    ...
    private Vec3d getVec3(WurstplusEventRender event, EntityPlayer e) {
        float pt = event.get_partial_ticks();
    ...
    private void drawSkeleton(WurstplusEventRender event, EntityPlayer e) {
        final Color rainbowColor1 = new Color(red.get_value(1), green.get_value(1), blue.get_value(1));
        final Color rainbowColor2 = new Color(rainbowColor1.getRed(), rainbowColor1.getGreen(), rainbowColor1.getBlue());
        double d3 = mc.player.lastTickPosX + (mc.player.posX - mc.player.lastTickPosX) * (double)event.get_partial_ticks();
        double d4 = mc.player.lastTickPosY + (mc.player.posY - mc.player.lastTickPosY) * (double)event.get_partial_ticks();
        double d5 = mc.player.lastTickPosZ + (mc.player.posZ - mc.player.lastTickPosZ) * (double)event.get_partial_ticks();
        ...
        if (entPos != null && e.isEntityAlive() && camera.isBoundingBoxInFrustum(e.getEntityBoundingBox()) && !e.isDead && e != mc.player && !e.isPlayerSleeping())
        {
            ...
            GlStateManager.color(rainbowColor2.getRed() / 255.0F, rainbowColor2.getGreen() / 255.0F, rainbowColor2.getBlue() / 255.0F, alpha.get_value(1) / 255.0F);
            ...
            float xOff = e.prevRenderYawOffset + (e.renderYawOffset - e.prevRenderYawOffset) * event.get_partial_ticks();
